...
for index, row in anue_df.iterrows():
    title = row["Title"]
    content = row["Content"]
    anue_data += f"新聞出處:鉅亨網 新聞標題: {title}  內容: {content}\n\n"

for index, row in MoneyDJ_df.iterrows():
    title = row["Title"]
    content = row["Content"]
    MoneyDJ_data += f"新聞出處:MoneyDJ理財網 新聞標題: {title}  內容: {content}\n\n"

for index, row in Franklin_df.iterrows():
    title = row["Title"]
    content = row["Content"]
    Franklin_data += f"出處:富蘭克林‧國民的基金\n國際股債市焦點:\n標題: {title}  內容: {content}\n\n"

data = anue_data + MoneyDJ_data + Franklin_data

# 不同角色 prompt
role_prompts = {
    "beginner": "報告對象為投資新手(投資經驗在5年以內)，故報告內請儘量不要包含專業術語，而是以相對容易理解的方式，描述並解釋各種市場現象，並且由於對象缺乏基礎所以請有條理地講述原因。",
    "intermediate": "報告對象為投資老手(投資經驗在5年至15年)，他對於財報中各項指標的意義和市場走向有基本的了解。",
    "advanced": "報告對象為投資高手(投資經驗在15年以上)，他對於金融市場，財報中各項指標的意義和市場走向非常了解。報告內可含有一些公司營運相關數據、指標，如果有報導中有列出實際數字的話(例如: 營收突破3億)請在報告中列出實際數字。"
}

def generate_market_prompt(role_prompt, data):
    return f"""
你是一個資深的金融業者兼理專，請根據以下幾篇新聞生成一份完整專業且易於閱讀的金融市場分析報告：
{data}
{role_prompt}
...
1.請確保文章內容具體，避免模糊的表述。提供具體的數據、指標、趨勢分析。例如，替代 "我們見證了一系列的高潮與低谷" 這種抽象語句，應該更具體描述實際的市場波動，如 "美股在過去三個月內出現了5%的下跌幅度，其中科技股跌幅最大，尤其是XX公司下降了8%"。
2.用詞請盡量專業豐富，文章的評論需要具體且有深度分析，並且提供具體市場事件和數據支持。
3.請引用具體的數據來支撐每個市場的分析，特別是股票市場、債券市場和大宗商品市場，並且提供具體的價格變化和趨勢。結論部分請根據數據給出有深度的見解。
4.請特別注意每一段的結構應該是(引用事件)->(加入事件相對的數據)->導致甚麼樣的結果
5.每一段中有引用新聞內容或數據的部分，需在每段結束後以括號標明該內容的新聞出處以及新聞標題在句號後面，格式說明:"段落內容...。('新聞出處' '新聞標題')"，若有多個新聞以頓號隔開。例如:(新聞出處A 新聞標題XX 、 新聞出處B 新聞標題YY)
